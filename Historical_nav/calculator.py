...
def calculate_returns(df, return_file_path="returns.csv"):
    df = df.copy()
    ...
    if current_year_start > nav_wide.index[0]:
        ...
        returns['YTD Return'] = round(((latest_nav - ytd_nav) / ytd_nav * 100), 2)
    else:
        returns['YTD Return'] = None

    # Step 7: Total Return from first available NAV
    first_nav = nav_wide.iloc[0]
    returns['Total Return'] = round(((latest_nav - first_nav) / first_nav * 100), 2)
